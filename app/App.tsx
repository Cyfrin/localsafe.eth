...
import SnapRequestClient from "./safe/[address]/snap-request/SnapRequestClient";
...
function SnapRequestWrapper() {
  const { address } = useParams<{ address: string }>();
  return <SnapRequestClient safeAddress={address as `0x${string}`} />;
}
    ...
    <HashRouter>
      <Routes>
        <Route element={<Layout />}>
          ...
          <Route path="/safe/:address/snap-request" element={<SnapRequestWrapper />} />
        </Route>
      </Routes>
    </HashRouter>
  );

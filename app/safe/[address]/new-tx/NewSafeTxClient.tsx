...
import { encodeCalldataFromAbi, getFunctionSignature } from "@/app/utils/abiEncoder";
 ...
 * Only includes nonpayable and payable functions (excludes view/pure which don't modify state)
 *
 * @param abi ABI array
...
export default function NewSafeTxClient({ safeAddress }: { safeAddress: `0x${string}` }) {
   ...
   * Uses debouncing to avoid repeated error toasts while typing
   */
  useEffect(() => {
    ...
    // Only proceed if method is selected and ABI is available
    if (!selectedMethod || !abiJson) {
      return;
    }

    // For functions with no arguments, generate calldata immediately (no debounce needed)
    if (methodInputs.length === 0) {
      const result = encodeCalldataFromAbi(abiJson, selectedMethod, {});
      if (result.success) {
        setData(result.data);
    ...
    // For functions with arguments, check if all required inputs have values
    ...
    // Debounce encoding attempt - wait 500ms after user stops typing
  ...
  async function handleBuildSafeTransaction() {
    setError(null);
    try {
      // Map transactions to the SafeTransaction input format
                ...
                {abiMethods.length > 0 && (
                  <div className="mt-2" data-testid="new-safe-tx-abi-methods-select-row">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        // Clear calldata and error tracking when switching methods
                        setData("");
                        setAutoGeneratedCalldata(null);
                        setCalldataManuallyEdited(false);
                        lastErrorRef.current = null;
                        // Clear any pending encoding timeout
                        if (encodeTimeoutRef.current) {
                          clearTimeout(encodeTimeoutRef.current);
                          encodeTimeoutRef.current = null;
                        }
